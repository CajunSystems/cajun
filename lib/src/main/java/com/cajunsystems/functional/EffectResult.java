...
public sealed interface EffectResult<State, Result> {
     ...
     */
    record Success<State, Result>(State state, Result resultValue) implements EffectResult<State, Result> {}
    ...
    /**
     * Effect execution failed with an error.
     * 
     * @param state The state at the time of failure
     * @param errorValue The error that occurred
     */
    record Failure<State, Result>(State state, Throwable errorValue) implements EffectResult<State, Result> {}
     ...
     */
    static <S, R> EffectResult<S, R> success(S state, R value) {
     ...
     */
    static <S, R> EffectResult<S, R> noResult(S state) {
     ...
     */
    static <S, R> EffectResult<S, R> failure(S state, Throwable error) {
    ...
    default Optional<Throwable> error() {
        return this instanceof Failure<State, Result> failure 
     ...
     */
    default <R2> EffectResult<State, R2> map(Function<Result, R2> f) {
        return switch (this) {
            case Success<State, Result> success -> 
                new Success<>(success.state(), f.apply(success.resultValue()));
            case NoResult<State, Result> noResult -> 
                new NoResult<>(noResult.state());
            case Failure<State, Result> failure -> 
     ...
     */
    default <S2> EffectResult<S2, Result> mapState(Function<State, S2> f) {
        return switch (this) {
            case Success<State, Result> success -> 
     ...
     */
    default EffectResult<State, Result> recover(Function<Throwable, Result> f) {
        return switch (this) {
            case Failure<State, Result> failure -> 
     ...
     */
    default EffectResult<State, Result> ifSuccess(Consumer<Result> action) {
        if (this instanceof Success<State, Result> success) {
     ...
     * @return This result unchanged
     */
    default EffectResult<State, Result> ifFailure(Consumer<Throwable> action) {
        if (this instanceof Failure<State, Result> failure) {
     ...
     * @return This result unchanged
     */
    default EffectResult<State, Result> ifState(Consumer<State> action) {
    ...
    default <T> T fold(
        ...
        return switch (this) {
            case Success<State, Result> success -> onSuccess.apply(success.resultValue());
            case NoResult<State, Result> noResult -> onNoResult.apply(noResult.state());
            case Failure<State, Result> failure -> onFailure.apply(failure.errorValue());
        };
    }
    ...
    default Result getOrThrow() throws Throwable {
        return switch (this) {
            case Success<State, Result> success -> success.resultValue();
            case NoResult<State, Result> noResult -> 
                throw new IllegalStateException("No result value present");
            case Failure<State, Result> failure -> 
                throw failure.errorValue();
        };
    }
    ...
    default String toDebugString() {
        return switch (this) {
            ...
            case Failure<State, Result> failure -> 
                String.format("Failure(state=%s, error=%s)", failure.state(), failure.errorValue().getMessage());
        };
    }
}
